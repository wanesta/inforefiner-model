...
double *LinearRegression::gradient_descent(double x[], double y[], double alpha, int iters, double *J, int m) {
    ...
    for (int i = 0; i < iters; i++) {
        ...
        double *diff = LinearUtil::array_diff(predictions, y, m);
        double *error_x1 = diff;
        double *error_x2 = LinearUtil::array_multiplication(diff, x, m);
        theta[0] = theta[0] - alpha * (1.0 / m) * LinearUtil::array_sum(error_x1, m);
        theta[1] = theta[1] - alpha * (1.0 / m) * LinearUtil::array_sum(error_x2, m);
...
double LinearRegression::compute_cost(double x[], double y[], double theta[], int m) {
    double *predictions = calculate_predictions(x, theta, m);
    double *diff = LinearUtil::array_diff(predictions, y, m);
    double *sq_errors = LinearUtil::array_pow(diff, m, 2);
    return (1.0 / (2 * m)) * LinearUtil::array_sum(sq_errors, m);
}
